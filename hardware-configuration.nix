{ config, lib, pkgs, ... }:
# This hardware configuration is meant to read a data.json file.
# That file contains all the hard drive information in an easy to read setup.
# This code will also setup spin-down timers for the drives. This was actually
# a nightmare to setup, and I don't know which parts of this is actually accomplishing 
# the task
...
let
  ...
  extractDevices = lib.concatLists (lib.mapAttrsToList (path: arrays:
    lib.concatLists (lib.mapAttrsToList (name: value:
      ...
      in
      [ "${pkgs.hdparm}/bin/hdparm -S 242 ${cleanedDevice}" ]
    ) arrays)
  ) jsonData);
  powerManagementCommands = builtins.concatStringsSep "\n" extractDevices;

  # Generate the power-up commands
  generateHdparmConfig = lib.concatLists (lib.mapAttrsToList (path: arrays:
    lib.concatLists (lib.mapAttrsToList (name: value:
      let
        # Remove the '-part1' suffix if it exists
        cleanedDevice = lib.replaceStrings ["-part1"] [""] value.device;
      in
      [ "${cleanedDevice} { spindown_time = 242; }" ]
    ) arrays)
  ) jsonData);
